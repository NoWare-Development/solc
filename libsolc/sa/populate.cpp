...
void
SemanticAnalyzer::populate_enums (
  ...
  for (auto &e : enum_asts)
    {
      ...
      struct PendingElement
      {
        ...
        PendingElement (const std::string &name, Enum::Element element,
                        const AST *ast_ptr)
            : name (name), element (std::move (element)), ast_ptr (ast_ptr)
        {
        }
      };
          ...
          if (!child.children.empty ())
            {
              ...
              if (!expr_type->is_integer ())
                {
                  add_error (SAErrorType::ENUM_ELEM_EXPR_NOT_INT,
              ...
              elements.emplace_back (
                  elem_name, Enum::Element::create (expr_result), &child);
            }
          else
            {
              if (elements.empty ())
                {
                  elements.emplace_back (elem_name, Enum::Element::create (),
                                         &child);
                }
              else
                {
                  const auto last = std::prev (elements.end ());
                  elements.emplace_back (elem_name,
                                         Enum::Element::create (last->element),
                                         &child);
                }
            }
        }
      ...
      for (auto e : elements)
        {
          if (enum_symbol->has_element (e.name))
            {
              add_error (SAErrorType::ENUM_ELEM_REDEF,
                         e.ast_ptr->token_position);
              continue;
            }
...
void
SemanticAnalyzer::populate_structs (
    const std::vector<std::pair<Struct *, const AST *>> asts)
{
}

void
SemanticAnalyzer::populate_unions (
    const std::vector<std::pair<Union *, const AST *>> asts)
{
}

void
SemanticAnalyzer::populate_funcs (
