...
SemanticAnalyzer::is_expr_known_at_comptime(const AST &expr) const
{
  ...
  case ASTType::EXPR_OPERAND_IDENTIFIER: {
    return is_valid_literal(expr.value) ?
             SAError{} :
             SAError(SAErrorType::ID_IN_CONST_EXPR, expr.token_position);
  } break;
  ...
  case ASTType::PREFIX_EXPR: {
    ...
    for (size_t i = 1; i < expr_len; i++) {
      ...
      case ASTType::EXPR_PREFIX_OPERATOR_DEREF:
        return { SAErrorType::DEREF_IN_CONST_EXPR, prefix.token_position };
...
SemanticAnalyzer::get_type_from_expr_ast(const AST &expr, bool taking_address)
{
  ...
  case ASTType::EXPR_OPERAND_NUMFLOAT: {
    if (taking_address) {
      add_error(SAErrorType::TAKE_ADDRESS_OF_A_LITERAL, expr.token_position);
      return nullptr;
    }
    ...
    if (!expr.children.empty()) {
      ...
      if (!is_valid_typespec(typespec.value)) {
        add_error(SAErrorType::INVALID_TYPESPEC, typespec.token_position);
        return nullptr;
      }
      ...
      if (!typespec_type->is_floating() &&
        ...
        add_error(SAErrorType::INT_TYPESPEC_ON_FLOAT, typespec.token_position);
        return nullptr;
      }
  ...
  case ASTType::EXPR_OPERAND_CAST_TO: {
    ...
    add_error(SAErrorType::CANNOT_CAST, expr.token_position,
  ...
  case ASTType::EXPR: {
    ...
    if (rhs_type->is_floating() || lhs_type->is_floating()) {
      ...
      case ASTType::EXPR_BINARY_OPERATOR_XOR: {
        auto floating_position = rhs_type->is_floating() ? rhs.token_position :
                                                           lhs.token_position;
    ...
    if (!can_convert_types(rhs_type, lhs_type)) {
      add_error(SAErrorType::CANNOT_CONVERT_TYPES_IN_EXPR,
  ...
  case ASTType::PREFIX_EXPR: {
    ...
    for (size_t i = 1; i < expr_len; i++) {
      if (expr.children.at(i).type == ASTType::EXPR_PREFIX_OPERATOR_ADDRESS) {
        taking_address = true;
        break;
      }
    }
    ...
    for (size_t i = 1; i < expr_len; i++) {
      ...
      case ASTType::EXPR_PREFIX_OPERATOR_DEREF: {
        if (!operand_type->is_explicit_pointer()) {
          add_error(SAErrorType::DEREF_ON_NONPTR, prefix_operand.token_position,
                    (std::vector<std::shared_ptr<Type> >){ operand_type });
          return nullptr;
        }
      ...
      case ASTType::EXPR_PREFIX_OPERATOR_NEG: {
        if (!operand_type->is_integer() && !operand_type->is_floating()) {
          add_error(SAErrorType::NEG_ON_NONNUM, prefix_operand.token_position,
                    (std::vector<std::shared_ptr<Type> >){ operand_type });
          return nullptr;
        }
      } break;
...
size_t SemanticAnalyzer::get_expr_comptime_ivalue(const AST &expr) const
{
  switch (expr.type) {
  case ASTType::EXPR_OPERAND_NUM: {
