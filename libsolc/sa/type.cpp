...
std::shared_ptr<Type> SemanticAnalyzer::get_type_from_type_ast(const AST &type)
{
  switch (type.type) {
  ...
  case ASTType::TYPE_ARRAY: {
    ...
    if (type.children.size() > 1) {
      ...
      if (!expr_type->is_integer()) {
        add_error(SAErrorType::NONINT_IN_ARRAY_DECL, expr.token_position,
...
bool SemanticAnalyzer::is_basic_type(const std::string &type) const
{
...
bool SemanticAnalyzer::is_valid_typespec(const std::string &ts) const
{
...
bool SemanticAnalyzer::is_valid_literal(const std::string &literal) const
{
