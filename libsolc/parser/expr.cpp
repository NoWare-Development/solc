...
AST Parser::parse_expression(bool toplevel)
{
  VERIFY_POS(_pos);
  size_t start = _pos;
...
AST Parser::parse_expression_tree(bool toplevel)
{
...
AST Parser::parse_expression_operand()
{
  VERIFY_POS(_pos);
  auto cur = _tokens.at(_pos);
  ...
  case TokenType::NUMOCT: {
    ...
    if (_pos < _tokens.size()) {
      ...
      if (cur.type == TokenType::ID) {
        out_operand.append(
          AST(_pos++, ASTType::EXPR_OPERAND_NUMTYPESPEC, cur.value));
      }
    }
    return out_operand;
  }
...
bool Parser::validate_expression(const AST &expr_ast, size_t &invalid_pos,
                                 std::string &out_reason) const
{
  ...
  auto peek_ast_type = [](const AST &root, int pos,
                          size_t &tok_pos) -> ASTType {
...
AST Parser::pratt_parse_expression(const std::vector<AST> &in, size_t *pos,
                                   int min_bp) const
{
...
void Parser::get_binding_power(ASTType op_type, int &l_bp, int &r_bp) const
{
