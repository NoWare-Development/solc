...
struct Scope {
  ...
  Scope() = default;
  Scope(Kind kind, const std::string &name = {})
    : name(name)
    , kind(kind)
  {
  }
  ...
  const std::shared_ptr<Variable> &get_variable(const std::string &name) const;

  bool has_function(const std::string &name) const;
  void add_function(const std::string &name, std::shared_ptr<Function> f);
  const std::shared_ptr<Function> &get_function(const std::string &name) const;
...
struct ScopeStack {
  ...
  construct_path(const Scope *terminator = nullptr) const;
