...
static inline constexpr BuiltinTypeGroup
get_builtin_type_group (BuiltinType type)
{
  return static_cast<BuiltinTypeGroup> (((uint32_t)type >> 8) & 0xFF);
}

static inline constexpr size_t
get_builtin_type_size (BuiltinType type)
{
  return (uint32_t)type & 0xFF;
}

static inline constexpr size_t
get_builtin_type_index (BuiltinType type)
{
...
struct Type final
{
  struct FuncArg final
  {
    ...
    FuncArg (std::shared_ptr<Type> type,
             std::shared_ptr<AST> def_expr = nullptr)
        : type (type), def_expr (def_expr)
    {
    }
  };
  ...
  Type (std::vector<FuncArg> &&arguments, size_t pointer_indirection)
      : arguments (arguments), return_type (create_basic (BuiltinType::VOID)),
        pointer_indirection (pointer_indirection + 1),
        builtin_type (BuiltinType::FUNC)
  {
  }
  Type (std::shared_ptr<Type> return_type, std::vector<FuncArg> &&arguments,
        size_t pointer_indirection)
      : arguments (arguments), return_type (return_type),
        pointer_indirection (pointer_indirection + 1),
        builtin_type (BuiltinType::FUNC)
  {
  }
  ...
  create_basic (BuiltinType builtin_type, size_t pointer_indirection = 0)
  {
                                 ...
                                 size_t pointer_indirection = 0)
  {
    return std::make_shared<Type> (
        std::move (arguments), std::move (array_sizes), pointer_indirection);
  }
  static std::shared_ptr<Type>
  create_function_pointer (std::shared_ptr<Type> return_type,
                           std::vector<FuncArg> arguments,
