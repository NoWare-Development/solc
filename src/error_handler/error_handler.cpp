...
ErrorHandler::ErrorHandler(const std::string &filename, const std::string &src)
{
  _filename = filename;
  _src = src;
...
void ErrorHandler::add_tokens(const std::vector<solc::Token> &tokens)
{
  _tokens = tokens;
}
...
std::string ErrorHandler::get_highlighted_token(const solc::Token &tok,
                                                ESCColor color,
                                                ESCGraphics mode) const
{
...
std::string ErrorHandler::last_line() const
{
  return get_line(_nol - 1);
}

size_t ErrorHandler::get_number_length(long long num) const
{
  size_t len = 0;
  if (num == 0) {
    return 1;
